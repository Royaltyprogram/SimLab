		  ...
		  try {
			...
			if (simulationSnapshot.exists()) {
			  ...
			  setLikeCount(simulationData.likes || 0);
				...
				if (!simulationData.codeUrl) {
				  console.log("No code URL provided - showing placeholder");
				  setSimulationCode(() => SimulationPlaceholder);
				  return;
				}
			  
			  try {
				const response = await fetch(simulationData.codeUrl);
				if (!response.ok) {
				...
				try {
				  const ComponentFunction = new Function('React', 'useState', 'useEffect', 'useRef', `
					${code}
					return ${simulation.componentName || 'SimulationComponent'};
				  `);
				...
				console.error("Error fetching simulation code:", error);
				setSimulationCode(() => SimulationPlaceholder);
			  }
			} else {
				setSimulationCode(() => SimulationPlaceholder);
				...
				<div className="max-w-5xl mx-auto">
				  <div className="flex flex-col gap-6">
					...
					<div className="py-4 border-b border-gray-200 flex justify-between items-center">
					  ...
					  {simulation.codeUrl ? (
						<a 
						  href={simulation.codeUrl.replace('/raw/', '/blob/')}
						  target="_blank" 
						  rel="noopener noreferrer"
			  ...
			  <div className="py-4">
				...
				<div className="min-h-[200px] border border-gray-200 rounded-lg p-4">
				  ...
				  <div className="mt-6 pt-6 border-t border-gray-200">
					...
					<a 
					  href="https://github.com/your-org/simlab"
					  target="_blank"
					  rel="noopener noreferrer"
